...
echo "▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
"
echo [`date +"%Y-%m-%d %H:%M:%S"`] "STARTING DOWNSTREAM ANALYSIS"

echo [`date +"%Y-%m-%d %H:%M:%S"`] "STARTING LNCRNA TARGET PREDICTION BY LncTar"
cd $LncTar
$lnctarr -p 1 -l $de_klnc/DE_lnc.fa -m $de_mrna/DE_mrna.fa -d -0.1 -s F -o Novel_All_Targets.txt
echo [`date +"%Y-%m-%d %H:%M:%S"`] "TARGET PREDICTION COMPLETE"
echo [`date +"%Y-%m-%d %H:%M:%S"`] "STARTING FUNCTIONAL ANNOTATION BY SEEKR"
cd $SEEKR
for i in {1..8}
do
seekr_kmer_counts $de_klnc/DE_lnc.fa -o kmer_counts_$i.csv -k $i
done
...
cp $de_klnc/DE_lnc.fa $lpic
mv $lpic/DE_lnc.fa $lpic/Novel_LncRNAs.fa
python3 $capsule
...
cd $miranda
$miraanda $mirna $de_klnc/DE_lnc.fa -out DEklncRNA_miRNA.txt
